...
func AboutHandler(about *About) func(httpResp http.ResponseWriter, httpReq *http.Request) {
	return func(httpResp http.ResponseWriter, httpReq *http.Request) {
		...
		if err != nil {
			http.Error(httpResp, fmt.Sprintf("Failed to encode %s. Something very wrong is going on.", httpReq.URL.Path),
				http.StatusServiceUnavailable)
			return
		}
		httpResp.Header().Set("Link", "<https://schema.org/WebAPI.jsonld>; rel=\"http://www.w3.org/ns/json-ld#context\"; type=\"application/ld+json\"")
		httpResp.Header().Set("Cache-Control", "max-age=60")
